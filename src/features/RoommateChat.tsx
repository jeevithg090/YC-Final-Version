...
const RoommateChat: React.FC = () => {
  ...
  const chatId = (params as any).chatId;
  const listing = (params as any).listing;
  const posterId = (params as any).posterId;
  const currentUserId = (params as any).currentUserId || auth.currentUser?.uid;
  ...
  useEffect(() => {
    ...
    const messagesRef = collection(db, 'roommate_chats', chatId, 'messages');
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (user.photoURL) {
        msg.senderAvatar = user.photoURL;
      }
  ...
  const renderMessage = ({ item }: { item: Message }) => {
    // Right: current user, Left: poster
  ...
  // UI: Listing info at the top
  const renderListingInfo = () => (
    <View style={styles.productInfoContainer}>
      {listing?.details?.photos?.[0] ? (
        <Image source={{ uri: listing.details.photos[0] }} style={styles.productImage} />
      ) : (
        <Ionicons name="bed-outline" size={56} color="#C7C7CC" style={{ backgroundColor: '#F2F2F7', borderRadius: 8 }} />
      )}
      <View style={{ flex: 1, marginLeft: 12 }}>
        <Text style={styles.productTitle}>{listing?.lookingFor || 'Roommate Listing'}</Text>
        <Text style={styles.productPrice}>₹{listing?.rentPerMonth}</Text>
        <Text style={styles.productMeta}>Posted by: {listing?.name || listing?.alias || 'User'}</Text>
      </View>
    </View>
  );
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <Text style={styles.headerTitle}>Roommate Chat</Text>
        <View style={{ width: 38 }} />
      </LinearGradient>
      {renderListingInfo()}
      {/* Chat messages */}
